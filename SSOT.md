...
- 루트 경로(`/`)는 앱 소개 랜딩페이지를 렌더링한다.
- 랜딩페이지 구성: 히어로 섹션, 기능 소개 섹션, 푸터(정책/지원 링크).
- 랜딩페이지는 정적 HTML로 빌드하며, 외부 JS/API 의존 없이 동작한다.
- 이미지 에셋은 `public/landing/` 하위에 저장한다.
